...
Core treats `.hint` as a compatibility alias for `.notes`, using the theme-aware muted color and normal weight across Morelord apps and chat cards. Prefer `.notes` in new templates. Existing consumers include Marketplace shop configuration and Craftworks recipe guidance; no template migration is required.

In verified Dev1, import `helperTextCheck` from `./modules/morelord-core/scripts/testing/helper-text.js` and pass it to `runInGameTests({ checks: [helperTextCheck] })`. It compares computed color and weight in standard forms and chat cards, then removes its temporary DOM fixture. Run in each theme. Live verification remains outstanding when the active world cannot be verified as Dev1.

Craftworks and Journeys each export `helperTextCheck` from their own `scripts/testing/helper-text.mjs`. Add these checks to Core’s shared runner in verified Dev1. They render actual settings and workflow templates and compare every fixture paragraph’s computed color, weight, and opacity to Core’s muted style; they clean up their DOM without saving any settings. `node tools/verify-helper-text.mjs` runs the same checks offline in both themes with installed module styles and records screenshots and results. This extends the earlier generic helper-only check to actual consumer markup.

September 24 helper-text follow-up: verified the live join-page and loaded world ID as `dev1`, then ran Core's shared runner on Foundry 14.368 / D&D5e 6.0.3. All seven checks passed, including the Core helper/section-subtitle fixture and actual Craftworks and Journeys template checks. Report: `test/in-game-reports/2026-09-24-helper-text-live.json`. Both offline themes passed, with Craftworks recipe-editor screenshots inspected. All 270 Node checks across Core (67), Craftworks (71), and Journeys (132), plus the design-system scan, passed. Full live theme/resizing coverage is not claimed. Refresh existing clients to load the revised CSS and templates.

Single-action chat width: in Dev1, import `singleActionControlsCheck` from `scripts/testing/single-action-controls.js` and run it with Core's `runInGameTests({ checks: [singleActionControlsCheck] })`. It checks Long Rest fills 260px and 340px card rows while DIS/Roll/ADV remain side by side, and removes its temporary DOM fixture. No world documents are changed.

September 24 section-text correction: the user clarified that section subtitles must use the main text color (white in the dark theme), while plain paragraphs inside sections must use the muted brown color. This supersedes the earlier muted-subtitle expectation. Core now owns both defaults, including unmarked body paragraphs; semantic callout/status colors remain intact. The shared and module-owned regressions distinguish header subtitles from body text and include the Harvest creature-selection section. Offline dark/light checks and the inspected Harvest screenshot pass.

The corrected live Dev1 run passed all seven checks on Foundry 14.368 / D&D5e 6.0.3; see test/in-game-reports/2026-09-24-section-text-correction.json. Core's 67 Node tests and the design-system scan also passed. Existing clients need a refresh to load the corrected shared CSS.

September 24 release verification (September 25 UTC): all 15 shared and owning-module checks passed in Dev1 on Foundry 14.368 / D&D5e 6.0.3. Coverage includes helper text, single-action controls, Journey planner/footer and watch labels, Marketplace manual shop policies and rarity prices (including Grenade, Fragmentation), and Craftworks generator controls and single-roll harvesting. Report: test/in-game-reports/2026-09-25-pending-release-validation.json. All 458 automated tests across eight active code modules and Core's design-system check passed. Marketplace now enforces its existing buyable item types, preventing unpriced feats or spells from becoming products. Full multiplayer/theme/zoom coverage is not claimed by this release run.
